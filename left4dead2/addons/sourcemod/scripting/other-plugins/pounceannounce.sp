...
public void Event_PlayerPounced(Event event, const char[] name, bool dontBroadcast) 
{
	...
	if(distance >= g_iMinPounceDistance && dmg >= g_iMinPounceAnnounce)
	{
		...
		#endif
		FormatEx(pounceLine,sizeof(pounceLine),"\x03[SM] %s \x01hizo pounce a \x05%s \x01de \x03%.01f \x01de salud.(Max: \x04%d\x01)", attackerName,victimName,dmg,g_iMaxPounceDamage + 1);
		...
		if(g_iShowDistance > 0)
		{
			switch(g_iShowDistance)
			{
				case (Units):
				{
					Format(distanceBuffer,sizeof(distanceBuffer)," ",distance);
				}
				case (UnitsAndFeet):
				{ //units / 16 = feet in game
					Format(distanceBuffer,sizeof(distanceBuffer)," ",distance, distance / 16);
				}
				case (UnitsAndMeters):
				{	//0.0213 = conversion rate for units to meters
					Format(distanceBuffer,sizeof(distanceBuffer)," ",distance, distance * 0.0213);
				}
				case (Feet):
				{
					Format(distanceBuffer,sizeof(distanceBuffer),"  ", distance / 16); 
				}
				case (Meters):
				{
					Format(distanceBuffer,sizeof(distanceBuffer)," ", distance * 0.0213);
				}
			}
