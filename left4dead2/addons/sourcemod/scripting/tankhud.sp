...
public void OnPluginStart()
{
	...
	g_hPanelTitle = CreateConVar("tank_hud_title", " HUD del Tank ", "Tank 状态面板标题", CVAR_FLAG);
...
public Action Timer_RefreshPanel(Handle timer)
{
	...
	if (!IsFakeClient(tankClient))
	{
		...
		FormatEx(info, sizeof(info), "▶ Controlado por ：%s", name);
	}
	else { FormatEx(info, sizeof(info), "▶ Controlado por：%N (AI)", tankClient); }
	...
	if (!IsClientIncapped(tankClient))
	{
		...
		// 除零异常
		FormatEx(info, sizeof(info), "▶ Salud：%d / %.1f%%", health, maxHealth > 0 ? 100.0 * health / maxHealth : 0.0);
	}
	else { FormatEx(info, sizeof(info), "▶ Estado：Muerto "); }
	tankHud.DrawText(info);
	// 控制权显示
	...
	FormatEx(rage, sizeof(rage), "▶ CONTROL：%d%% (%s)", GetTankFrustration(tankClient), info);
	...
	if (GetEntityFlags(tankClient) & FL_ONFIRE)
	{
		if (!IsClientIncapped(tankClient))
		{
			FormatEx(info, sizeof(info), "▶ Estado: Ardiendo - muerte en %.1f segundos", health / tankBurnDuration.FloatValue);
		}
		else
		{
			FormatEx(info, sizeof(info), "▶ Estado: Cocinado - Muerto");
		}
	}
	else { FormatEx(info, sizeof(info), "▶ Estado: Esta bajando pepa"); }
	tankHud.DrawText(info);
	// 是否被胆汁
	if (inVomit[tankClient]) { FormatEx(info, sizeof(info), "▶ Estado: Lo cumearon"); }
	else { FormatEx(info, sizeof(info), "▶ Estado: Tranqui"); }
	tankHud.DrawText(info);
	// 网络状态
	if (!IsFakeClient(tankClient))
	{
		FormatEx(info, sizeof(info), "▶ Ping：%dms", RoundToNearest(GetClientAvgLatency(tankClient, NetFlow_Both) * 100.0));
	}
	else { FormatEx(info, sizeof(info), "▶ Ping：No hay señal (AI)"); }
	tankHud.DrawText(info);
	// 将面板发送至旁观者与感染者团队
	for (i = 1; i <= MaxClients; i++)
	{
