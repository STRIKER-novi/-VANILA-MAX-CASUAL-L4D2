	...
	if (IsAiTank(client))
	{
		switch (g_iMessageType)
		{
			case 1:
			{
				...
				{
					CPrintToChatAll("El Tank ha aparecido! (Controlado por: %s)", sTankName);
				}
				else
				{
					CPrintToChatAll("Tank ：%s) ", sTankName);
				}
			}
			case 2:
			{
				PrintHintTextToAll("[Tank]：%s ！", sTankName);
			}
			case 3:
			{
				PrintCenterTextAll("[Tank] ：%N ！", sTankName);
			}
		}
		if (g_iPlaySound == 1)
		{
...
bool IsAiTank(int tank)
{
	...
	{
		FormatEx(sTankName, sizeof(sTankName), " %N", tank);
		return true;
	}
